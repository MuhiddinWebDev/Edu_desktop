...
<template>
  <div class="box">
      ...
      <div class="search-action">
        <div class="search-action_item" v-if="findRole == 'SuperAdmin'">
          <n-input-group>
            ...
            <n-select @update:value="selectBranch" :options="BranchOptions" value-field="id"
              v-model:value="sendData.filial_id" label-field="name" filterable clearable></n-select>
          </n-input-group>
        </div>
        <div class="search-action_item">
          <n-input-group>
            <n-input-group-label>F.I.O</n-input-group-label>
            <n-select @update:value="showOneUser" :options="UserOptions" v-model:value="sendData.id"
              :render-label="renderUser" :render-tag="renderUserSelect" value-field="id" label-field="fullname" filterable
              clearable></n-select>
          </n-input-group>
        </div>
      </div>
    ...
    <div class="box-table">
      <n-data-table :loading="loadingRef" :columns="columns" :data="data" :pagination="pagination" :bordered="true"
        :single-line="false" style="min-width: 1000px" max-height="calc(100vh - 300px)" :row-props="rowProps"
        :row-class-name="rowClassName" size="small">
      </n-data-table>
    </div>
  </div>
  ...
  <section>
    <n-modal transform-orign="center" v-model:show="showCreate" :mask-closable="false">
      <n-card style="max-width: 700px; width: calc(100vw - 15px)" title="Admin qo'shish" :bordered="false" size="large"
        role="dialog" aria-modal="true" closable @close="modalClose('create')">
        <ModelForm @close="modalClose('create')" @create="createModel" :defaultObeject="defaultObeject" type="create" />
      </n-card>
    </n-modal>
    <n-modal transform-orign="center" v-model:show="showUpdate" :mask-closable="false">
      <n-card style="max-width: 700px; width: calc(100vw - 15px)" title="Admini o'zgartirish" :bordered="false"
        size="large" role="dialog" aria-modal="true" closable @close="modalClose('update')">
        <ModelForm @close="modalClose('update')" type="update" :id="updateId" @update="updateModel" />
      </n-card>
    </n-modal>
    <n-drawer v-model:show="showCreateMore" width="100vw" height="calc(100vh - 120px)" placement="top">
      <n-drawer-content title="Adminlar qo'shish" closable>
        <ModelFormMore @close="modalClose('more-create')" @create="createMoreModel" type="create" />
      </n-drawer-content>
    </n-drawer>
    <!-- create more end -->
  </section>
